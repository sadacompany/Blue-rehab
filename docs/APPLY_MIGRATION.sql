-- ============================================================================
-- Blue Rehab — apply in Supabase → SQL Editor → New query
-- Project: lfuuptigzjocgewhrmkt
--
-- Verified against the live database on 2026-08-04: BOTH migrations below are
-- missing. `bookings.meeting_url` does not exist (42703) and
-- `create_booking_with_payment` is not in the schema cache (PGRST202).
--
-- ORDER MATTERS. 20260803090000 grants patients a full-row UPDATE on bookings;
-- 20260804120000 then revokes it and re-grants only the meeting columns. Running
-- them out of order leaves patients able to edit their own price and status.
--
-- STEP 1 must return ZERO rows before you run anything else.
-- ============================================================================


-- ---------------------------------------------------------------- STEP 1 ----
-- Run this SELECT alone. Zero rows = safe to continue.
-- Any rows returned mean two live bookings share a slot, and the unique index
-- in STEP 3 will fail. Cancel the duplicates first, then re-run this.
...
-- ---------------------------------------------------------------- STEP 2 ----
-- 20260803090000_booking_meeting_link.sql
...
-- ---------------------------------------------------------------- STEP 3 ----
-- 20260804120000_booking_payment_flow.sql
